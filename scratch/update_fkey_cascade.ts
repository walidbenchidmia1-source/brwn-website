...
const SQL = `
-- Modify foreign key on inventory_history to cascade delete
...
async function run() {
  console.log("Connecting to Supabase Database to update foreign key constraint on inventory_history...");
  ...
  try {
    ...
    console.log("✅ Foreign key constraint updated to ON DELETE CASCADE on inventory_history!");
  } catch (err: any) {
    console.error("❌ Failed to update foreign key constraint:", err.message || err);
    process.exit(1);
  } finally {
    await client.end();
  }
